    ...
    `<h3>${category}</h3><br><span>(${arr.length} items)`
        ...
        function shopitem(el){
            console.log(el);
        // localStorage.setItem("payfor",JSON.stringify(el));
        // window.location="pay.html"
    }
        function catchcat(cat){
            let category=cat;
        console.log(category)
        // let ctg = fetchcategory(cat)
        // localStorage.setItem("category", JSON.stringify(ctg));
        // window.location = "products.html"
    }
        ...
        console.log(n1[0].category);

        document.querySelector(".cd11").append(hp1name);
        document.getElementById("cd12").innerHTML =
        `<button onclick="shopitem(${n1[0]})" class="hp1-shop1-btn">Shop Now</button>
        <button onclick="catchcat(${n1[0].category})" class="hp1-view1-btn">View More</button>`
